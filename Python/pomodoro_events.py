...
class CheckStudyingCommand(Event):
    ...
    def run_event(self, event_queue: EventQueue):
        items = []
        with open(os.path.abspath('Python/database/studying.csv'), 'r') as users:
            for user in users:
                items.append(user)
        ...
        if studying:
            event_queue.add(MessageEvent(self.start_time, self.length,
                                         "Yes, that user is currently studying! You probably shouldn't bother them!", self.user_id))
        else:
            event_queue.add(MessageEvent(self.start_time, self.length,
                                         "No, that user isn't studying! Feel free to give them a call!", self.user_id))
...
class PomodoroCommand(Event):
    ...
    def run_event(self, event_queue):
        info = os.path.abspath('Python/database/studying.csv')
        start_time = datetime.datetime.now() + datetime.timedelta(minutes=25)
        reminder_event = PomodoroBreakReminder(
            start_time, datetime.timedelta(), self.user_id)
        ...
        reminder = PomodoroLongBreakReminder(
            start_time, datetime.timedelta(), self.user_id)
        ...
        items = []
        with open(info, 'r') as studying_users:
        ...
        if str(self.user_id) not in items:
            ...
            with open(info, 'w', newline='') as studying_users:
...
class PomodoroRepeated(Event):
    ...
    def create_next_repeated_event(self) -> Event:
        ...
        clone.start_time = clone.start_time + \
