...
class MusicReqCommand(Event):
    ...
    def run_event(self, event_queue: EventQueue):
        music = []
        with open(os.path.abspath('Python/database/music.csv'), 'r') as file:
        ...
        event_queue.add(MessageEvent(self.start_time,
                                     self.length, message, self.user_id))
...
class MeditationReqCommand(Event):
    ...
    def run_event(self, event_queue: EventQueue):
        meditations = []
        with open(os.path.abspath('Python/database/meditation.csv'), 'r') as file:
            reader = csv.reader(file)
            for row in reader:
