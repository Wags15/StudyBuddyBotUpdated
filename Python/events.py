...
class EventQueue:
    ...
    def add(self, event: Event):
        ...
        self.events.sort(key=lambda e: e.start_time -
                         datetime.datetime.now(), reverse=True)

    def add_list(self, new_events: list[Event]):
        for e in new_events:
            self.events.append(e)

        self.events.sort(key=lambda ev: ev.start_time -
                         datetime.datetime.now(), reverse=True)
...
class EyeStrainReminder(Event):
    ...
    def run_event(self, event_queue: EventQueue):
        event_queue.add(MessageEvent(datetime.datetime.now(),
                                     datetime.timedelta(), self.reminder_text, self.user_id))
...
class EyesCommand(Event):
    def run_event(self, event_queue: EventQueue):
        turning_on = True
        users = []
        with open(os.path.abspath('Python/eyes_active_users.txt')) as active_users:
        ...
        if turning_on:
            ...
            reminder_event = EyeStrainReminder(
                new_start_time, datetime.timedelta(), self.user_id)
            repeated_event = RepeatedEvent(
                new_start_time, self.length, 1, reminder_event, True, self.user_id, datetime.timedelta(minutes=20))
            event_queue.add(repeated_event)

            with open(os.path.abspath('Python/eyes_active_users.txt'), 'a') as file:
                file.write(str(self.user_id) + '\n')
                file.close()

            event_queue.add(MessageEvent(datetime.datetime.now(), datetime.timedelta(
            ), "Eye Strain Reduction Session Has Begun! Type +eyes To End It.", self.user_id))
        else:
                ...
                if isinstance(event_queue.events[i], RepeatedEvent):
                    if isinstance(event_queue.events[i].event, EyeStrainReminder):
                        if event_queue.events[i].user_id == self.user_id:
                            event_queue.events[i] = NullEvent(
                                datetime.datetime.now(), datetime.timedelta(), self.user_id)
                            break
            ...
            event_queue.add(MessageEvent(datetime.datetime.now(), datetime.timedelta(
            ), "Eye Strain Reduction Session Has Ended! Type +eyes To Begin Again.", self.user_id))
