...
class GamingReminderEvent(Event):
    ...
    def run_event(self, event_queue):
        event_queue.add(MessageEvent(self.start_time,
                                     self.length, self.message, self.user_id))
...
class GamingCommand(Event):
    ...
    def run_event(self, event_queue: EventQueue):
        info = os.path.abspath('Python/database/gaming_users.csv')
        turning_on = True
        og = 'D:\HACKATHONS\The-Study-Buddy-Bot\Python\database\gaming_users.csv'

        with open(info, 'r') as users:
        ...
        if turning_on:
            ...
            event_queue.add(MessageEvent(datetime.datetime.now(),
                                         datetime.timedelta(), output, self.user_id))

            with open(info, 'a', newline='') as users:
                writer = csv.writer(users)
                writer.writerow([str(self.user_id)])

            new_start_time = self.start_time + datetime.timedelta(minutes=60)
            repeated_message_event = GamingReminderEvent(
                new_start_time, datetime.timedelta(), self.user_id)
            repeated_event = RepeatedEvent(new_start_time, datetime.timedelta(
            ), 1, repeated_message_event, True, self.user_id, datetime.timedelta(minutes=60))
            ...
            items = []
            with open(info, 'r', newline='') as users:
                reader = csv.reader(users)
                for row in reader:
                    items.append(row[0])
            ...
            with open(info, 'w', newline='') as users:
                writer = csv.writer(users)
                for item in items:
                ...
                if isinstance(event_queue.events[i], RepeatedEvent):
                    if isinstance(event_queue.events[i].event, GamingReminderEvent):
                        event_queue.events[i] = NullEvent(
                            self.start_time, self.length, self.user_id)
